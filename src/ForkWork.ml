open Printf
open ForkWorkHelpers

module D = Debug.Make(struct let name = "ForkWork" and default=`Debug end)
open D
;;
...
(*
 * Persistent fork. Tries to spawn a child repeatedly, waiting until
 * a child exits before retrying. XXX: This should also take free memory
 * into account; otherwise, we eventually end up forking a child when
 * just enough memory to begin execution is available, but then the
 * child allocates more memory either directly or by running the GC
 * (which unshares tons of pages) leading us into an out-of-memory
 * situation. The symptom of that normally is an IPC exception when
 * we try to read the result from the killed child (which might not
 * be the last child forked, obviously). We could probably estimate
 * the growth factor for any child and then only fork if there's
 * enough memory available for the child to grow. Another option
 * would also be to re-run any killed children when more memory
 * becomes available. Not sure it's worth the trouble at this point.
 *)
let rec pfork mgr tries =
  assert (tries > 0);
  try
    Unix.fork ()
  with Unix.Unix_error _ ->
    ...
    if (Hashtbl.length mgr.pending) > 0 then begin
      ignore(Unix.wait ());
      dprintf "Collecting after fork failure";
      collect_results mgr;
      pfork mgr (tries - 1)
    end else if tries = 1 then begin
      failwith "Unrecoverable fork error (tries exceeded)"
    end else begin
      failwith "Unrecoverable fork error (nothing to collect)"
    end

...
module D = Debug.Make(struct let name = "SymbEvalCopy" and default=`NoDebug end)
open D
...
sig
  ...
  (** Update memory *)
  val update_mem : t -> varval -> Ast.exp -> Ast.exp -> Ast.exp -> varval
  (** Lookup memory *)
  val lookup_mem : t -> varval -> Ast.exp -> Ast.exp -> Ast.exp
  ...
  let rec eval_expr delta expr =
	...
	| Reg 8 ->
		 ...
		 and endian_exp = symb_to_exp endian in
		   Symbolic (lookup_mem delta mem mem_arr endian_exp)
	  ...
	  and endian = symb_to_exp (eval_expr delta endian) in
	    (match t with
		   ...
		   let mem = eval_expr delta mem in
		     update_mem delta mem index value endian
	       | Reg _ ->
...
struct
  ...
  let rec update_mem delta mu pos value endian =
    (*pdebug "Update mem" ;*)
    match mu with
	  ...
	  match pos with
	    ...
	    | _ -> update_mem delta (conc2symb m v) pos value endian

  let rec lookup_mem delta mu index endian =
    (*pdebug "Lookup mem" ;*)
    match mu, index with
      ...
      | ConcreteMem(m,v),_ -> lookup_mem delta (conc2symb m v) index endian
